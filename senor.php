<!DOCTYPE html>
<html>
<head>
<title>Portofolio - fatinsomniac</title>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
<link href="https://fonts.googleapis.com/css?family=Montserrat" rel="stylesheet">
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
<link rel="stylesheet" href="./css/style-blog.css">
</head>
<body>

<div id="page">
<nav class="navbar navbar-expand-lg navbar-light bg-custom fixed-top">
  <a href="/"><img src="./img/logo/fav.png" class="" width="auto" height="60px" style="margin-top: 0;margin-left: 5%;"></a>
  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
    <img src="./img/burger-menu.svg">
  </button>

  <div class="collapse navbar-collapse topnav" id="navbarSupportedContent">
    <ul class="navbar-nav ml-auto nav-fill w-custom" style="margin-right: 5%">
      <li class="nav-item">
        <a class="nav-link" href="index.php">Home <span class="sr-only">(current)</span></a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="portofolio.php">Portofolio</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="resume.php">Resume</a>
      </li>
      <li class="nav-item">
        <a class="typeform-share button nav-link" href="https://fatinsomniac.typeform.com/to/EMsrEa" data-mode="popup" target="_blank">Contact </a><script> (function() { var qs,js,q,s,d=document, gi=d.getElementById, ce=d.createElement, gt=d.getElementsByTagName, id="typef_orm_share", b="https://embed.typeform.com/"; if(!gi.call(d,id)){ js=ce.call(d,"script"); js.id=id; js.src=b+"embed.js"; q=gt.call(d,"script")[0]; q.parentNode.insertBefore(js,q) } })() </script>
      </li>
    </ul>
  </div>
</nav>

 <!-- !PAGE CONTENT! -->
      <div class="w3-main w3-content w3-padding" style="max-width:1200px;margin-top: 13%">
        <div class="container">
          <div class="row">
            <div class="col-md-10 offset-md-1 titel-article">
            <h1>Website Senor Production</h1>
            <div class="sub-title mt-3">
                Posted at 2019-10-08 18:43:05 on ARTIKEL by Dede Dwi Narantika
            </div>
            <img src="./img/portofolio/senor/senor.png" width="100%" class="mt-3" style="border: 1px;border-color: #fff;">
            <div class="row mt-5">
              <div class="col-md-3 order-sm-12 order-12 order-md-1 order-lg-1 mb-3">
                <div id="tagar">
                  <h6>Hastag</h6>
                  <ul>
                    <li><a href="">#WebsiteCP</a></li>
                    <li><a href="">#SenorProduction</a></li>
                  </ul>
                </div>
                 <div id="favourite" class="mt-4">
                  <div class="share mt-4">
                    <a target="_blank" href="https://instagram.com/fatinsomniac"><i class="fa fa-instagram"></i></a>
                    <a target="_blank" href="https://twitter.com/fatinsomniac"><i class="fa fa-twitter"></i></a>
                    <a target="_blank" href="https://facebook.com/robotdede"><i class="fa fa-facebook-f"></i></a>
                    <a target="_blank" href="https://github.com/fatinsomniac"><i class="fa fa-github"></i></a>
                  </div>
                </div>
              </div>
      
              <div class="col-md-9 order-sm-1 order-1 order-md-12 order-lg-12">
      
              <p>
                Saya pernah magang disalah satu start-up di Jakarta Barat yang bernama Cozer ID anak perusahaan dari PT Senor Technology, di sana saya bekerja sebagai fullstack developer.
              </p>
              <p>
                Beberapa project pernah saya selesaikan di sana, salah satunya adalah membuat Website CP(Company Profile) untuk team produksi. Di dalam project ini saya berperan sebagai front-end, mengaplikasikan design yang sudah di buat oleh salah satu team produksi dari prototype lewat design adobe xd menjadi sebuah bentuk yang saya aplikasikan menggunakan html, css, javascript, dan php dengan back-end nya menggunakan laravel versi 5.8.
              </p>
              <p>
                Di bawah ini untuk tampilan menu Work.
              </p>
              <p>
                <img src="./img/portofolio/senor/senor2.png" alt="" width="100%" />
                <em>Menu Work</em>
              </p>
              <hr style="border: 2px solid #fff;margin-top: 1%;"><p><strong>Link Website : </strong> <a href="http://production.senor.co.id/" style="color: #43FFFE;">Senor Production</a></p>
              </div>
          </div>
        </div>
      </div>
  <!-- Footer -->
<footer class="w3-center" style="margin-top: 10%;">
  <p> &copy;fatinsomniac</p>
</footer>
</div>
<div id="loading">
    <div class="text-center" style="margin-top: 45%;">
      <span class="load">LOADING IDEA FOR YOU</span>
        <div class="col-xs-12 col-sm-12 progress-container">
          <center>
              <div class="progress active" style="height:10px; width: 250px;">
                  <div class="progress-bar progress-bar-primary" style="width:0%"></div>
              </div>
          </center>
        </div>
    </div>
  </div>

  <script src="https://code.jquery.com/jquery-3.3.1.min.js" integrity="sha256-FgpCb/KJQlLNfOu91ta32o/NMZxltwRo8QtmkMRdAu8=" crossorigin="anonymous"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js" integrity="sha384-ZMP7rVo3mIykV+2+9J3UJ46jBk0WLaUAdn689aCwoqbBJiSnjAK/l8WvCWPIPm49" crossorigin="anonymous"></script>
  <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js" integrity="sha384-ChfqqxuZUCnJSK3+MXmPNIyE6ZbWh2IMqE241rYiqJxyMiZ6OW/JmZQ5stwEULTy" crossorigin="anonymous"></script>
<script>
$(".progress-bar").animate({
    width: "100%"
  }, 1500 );
function onReady(callback) {
var intervalID = window.setInterval(checkReady, 1000);
function checkReady() {
if (document.getElementsByTagName('body')[0] !== undefined) {
  window.clearInterval(intervalID);
  callback.call(this);
}
}
}

function show(id, value) {
    document.getElementById(id).style.display = value ? 'block' : 'none';
}

  onReady(function () {
      show('page', true);
      show('loading', false);
});

  </script>   
</body>
</html>